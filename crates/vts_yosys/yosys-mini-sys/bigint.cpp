...
void BigUnsigned::divideWithRemainder(const BigUnsigned &b, BigUnsigned &q) {
  ...
  if (this == &q)
    throw "BigUnsigned::divideWithRemainder: Cannot write quotient and "
          "remainder into the same variable";
...
BigInteger::BigInteger(const Blk *b, Index blen, Sign s) : mag(b, blen) {
  ...
  case zero:
    if (!mag.isZero())
      throw "BigInteger::BigInteger(const Blk *, Index, Sign): Cannot use a "
...
BigInteger::BigInteger(const BigUnsigned &x, Sign s) : mag(x) {
  ...
  case zero:
    if (!mag.isZero())
      throw "BigInteger::BigInteger(const BigUnsigned &, Sign): Cannot use a "
            "sign of zero with a nonzero magnitude";
    sign = zero;
    break;
  case positive:
...
namespace {
template <class X, class UX> BigInteger::Blk magOf(X x) {
...
template <class X> X BigInteger::convertToUnsignedPrimitive() const {
  if (sign == negative)
...
template <class X, class UX> X BigInteger::convertToSignedPrimitive() const {
  if (sign == zero)
    return 0;
...
void BigInteger::divideWithRemainder(const BigInteger &b, BigInteger &q) {
  ...
  if (this == &q)
    throw "BigInteger::divideWithRemainder: Cannot write quotient and "
          "remainder into the same variable";
  if (this == &b || &q == &b) {
    BigInteger tmpB(b);
    divideWithRemainder(tmpB, q);
