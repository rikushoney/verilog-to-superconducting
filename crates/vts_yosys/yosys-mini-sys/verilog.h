...
public:
  ...
  void parse_verilog(const std::vector<VerilogFmtArg> &args, bool sformat_like,
                     int default_base, RTLIL::IdString task_name,
                     RTLIL::IdString module_name);
  std::vector<VerilogFmtArg> emit_verilog() const;

  void emit_cxxrtl(std::ostream &f,
...
struct AstSrcLocType {
  ...
  AstSrcLocType(int _first_line, int _first_column, int _last_line,
                int _last_column)
      : first_line(_first_line), last_line(_last_line),
        first_column(_first_column), last_column(_last_column) {}
};
...
struct AstNode {
  ...
  bool is_input, is_output, is_reg, is_logic, is_signed, is_string, is_wand,
      is_wor, range_valid, range_swapped, was_checked, is_unsized,
      is_custom_type;
  ...
  // if this is a multirange memory then this vector contains offset and length
  // of each dimension
  ...
  // this is used by simplify to detect if basic analysis has been performed
  // already on the node
  bool basic_prep;

  // this is used for ID references in RHS expressions that should use the "new"
  // value for non-blocking assignments
  bool lookahead;
  ...
  AstNode(AstNodeType type = AST_NONE, AstNode *child1 = nullptr,
          AstNode *child2 = nullptr, AstNode *child3 = nullptr,
  ...
  // simplify() creates a simpler AST by unrolling for-loops, expanding generate
  // blocks, etc. it also sets the id2ast pointers so that identifier lookups
  // are fast in genRTLIL()
  bool simplify(bool const_fold, int stage, int width_hint, bool sign_hint);
  void replace_result_wire_name_in_function(const std::string &from,
                                            const std::string &to);
  AstNode *readmem(bool is_readmemh, std::string mem_filename, AstNode *memory,
                   int start_addr, int finish_addr, bool unconditional_init);
  void expand_genblock(const std::string &prefix);
  void label_genblks(std::set<std::string> &existing, int &counter);
  void
  mem2reg_as_needed_pass1(dict<AstNode *, pool<std::string>> &mem2reg_places,
                          dict<AstNode *, uint32_t> &mem2reg_flags,
                          dict<AstNode *, uint32_t> &proc_flags,
                          uint32_t &status_flags);
  bool mem2reg_as_needed_pass2(pool<AstNode *> &mem2reg_set, AstNode *mod,
                               AstNode *block, AstNode *&async_block);
  ...
  bool replace_variables(std::map<std::string, varinfo_t> &variables,
                         AstNode *fcall, bool must_succeed);
  AstNode *eval_const_function(AstNode *fcall, bool must_succeed);
  bool is_simple_const_expr();

  // helper for parsing format strings
  Fmt processFormat(int stage, bool sformat_like, int default_base = 10,
                    size_t first_arg_at = 0);
  ...
  // all generated cell instances, etc. are written to the RTLIL::Module pointed
  // to by AST_INTERNAL::current_module
  RTLIL::SigSpec genRTLIL(int width_hint = -1, bool sign_hint = false);
  RTLIL::SigSpec
  genWidthRTLIL(int width, bool sgn,
                const dict<RTLIL::SigBit, RTLIL::SigBit> *new_subst_ptr = NULL);
  ...
  static AstNode *mkconst_bits(const std::vector<RTLIL::State> &v,
  ...
  // Helper for looking up identifiers which are prefixed with the current
  // module name
  std::string try_pop_module_prefix() const;

  // helper to clone the node with some of its subexpressions replaced with zero
  // (this is used to evaluate widths of dynamic ranges)
  ...
  // helper to set in_lvalue/in_param flags from the hierarchy context (the
  // actual flag can be overridden based on the intrinsic properties of this
  // node, i.e. based on its type)
  void set_in_lvalue_flag(bool flag, bool no_descend = false);
  void set_in_param_flag(bool flag, bool no_descend = false);

  // fix up the hierarchy flags (in_lvalue/in_param) of this node and its
  // children
  //
  // to keep the flags in sync, fixup_hierarchy_flags(true) needs to be called
  // once after parsing the AST to walk the full tree, then plain
  // fixup_hierarchy_flags() performs localized fixups after modifying
  // children/attributes of a particular node
...
// code
void process(RTLIL::Design *design, AstNode *ast, bool dump_ast1,
             bool dump_ast2, bool no_dump_ptr, bool dump_vlog1, bool dump_vlog2,
             bool dump_rtlil, bool nolatches, bool nomeminit, bool nomem2reg,
             bool mem2reg, bool noblackbox, bool lib, bool nowb, bool noopt,
             bool icells, bool pwires, bool nooverwrite, bool overwrite,
             bool defer, bool autowire);

// parametric modules are supported directly by the AST library
// therefore we need our own derivate of RTLIL::Module with overloaded virtual
// functions
struct AstModule : RTLIL::Module {
  AstNode *ast;
  bool nolatches, nomeminit, nomem2reg, mem2reg, noblackbox, lib, nowb, noopt,
      icells, pwires, autowire;
  ~AstModule() override;
  RTLIL::IdString derive(RTLIL::Design *design,
  ...
  derive(RTLIL::Design *design,
         ...
         bool mayfail) override;
  std::string
  derive_common(RTLIL::Design *design,
                const dict<RTLIL::IdString, RTLIL::Const> &parameters,
                AstNode **new_ast_out, bool quiet = false);
  void expand_interfaces(
      RTLIL::Design *design,
      const dict<RTLIL::IdString, RTLIL::Module *> &local_interfaces) override;
...
// set set_line_num and get_line_num to internal dummy functions (done by
// simplify() and AstModule::derive to control the filename and linenum
// properties of new nodes not generated by a frontend parser)
void use_internal_line_num();

// call a DPI function
AstNode *dpi_call(const std::string &rtype, const std::string &fname,
...
void explode_interface_port(AstNode *module_ast, RTLIL::Module *intfmodule,
...
std::string derived_module_name(
...
extern bool flag_dump_ast1, flag_dump_ast2, flag_no_dump_ptr, flag_dump_rtlil,
    flag_nolatches, flag_nomeminit;
extern bool flag_nomem2reg, flag_mem2reg, flag_lib, flag_noopt, flag_icells,
    flag_pwires, flag_autowire;
...
RTLIL::Module *process_and_replace_module(RTLIL::Design *design,
...
// this variable is set to a new AST_DESIGN node and then filled with the AST by
// the bison parser
