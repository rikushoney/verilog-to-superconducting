...
AST::Binding::Binding(RTLIL::IdString target_type, RTLIL::IdString target_name,
                      const AstNode &cell)
    : RTLIL::Binding(target_type, target_name), ast_node(cell.clone()) {
  // log_assert(cell.type == AST_CELL);
}

std::string AST::Binding::describe() const {
  std::ostringstream oss;
  oss << "directive to bind " << ast_node->str << " to " << target_name.str();
  if (!target_type.empty())
    oss << " (target type: " << target_type.str() << ")";
  return oss.str();
}
...
RTLIL::IdString new_id(std::string file, int line, std::string func) {
  ...
  return stringf("$auto$%s:%d:%s$%d", file.c_str(), line, func.c_str(),
                 autoidx++);
}
...
RTLIL::IdString new_id_suffix(std::string file, int line, std::string func,
  ...
  return stringf("$auto$%s:%d:%s$%s$%d", file.c_str(), line, func.c_str(),
                 suffix.c_str(), autoidx++);
}
...
void RTLIL::AttrObject::set_bool_attribute(const RTLIL::IdString &id,
                                           bool value) {
  if (value)
...
void RTLIL::AttrObject::set_string_attribute(const RTLIL::IdString &id,
                                             std::string value) {
  if (value.empty())
    attributes.erase(id);
  else
...
std::string
RTLIL::AttrObject::get_string_attribute(const RTLIL::IdString &id) const {
  std::string value;
  const auto it = attributes.find(id);
  if (it != attributes.end())
...
void RTLIL::AttrObject::set_strpool_attribute(const RTLIL::IdString &id,
...
void RTLIL::AttrObject::add_strpool_attribute(const RTLIL::IdString &id,
                                              const pool<std::string> &data) {
  pool<std::string> union_data = get_strpool_attribute(id);
...
pool<std::string>
RTLIL::AttrObject::get_strpool_attribute(const RTLIL::IdString &id) const {
...
void RTLIL::AttrObject::set_hdlname_attribute(
...
std::vector<std::string> RTLIL::AttrObject::get_hdlname_attribute() const {
  return split_tokens(get_string_attribute(ID::hdlname), " ");
}

void RTLIL::AttrObject::set_intvec_attribute(const RTLIL::IdString &id,
...
std::vector<int>
RTLIL::AttrObject::get_intvec_attribute(const RTLIL::IdString &id) const {
  std::vector<int> data;
...
bool RTLIL::Selection::selected_whole_module(
    const RTLIL::IdString &mod_name) const {
  if (full_selection)
    return true;
  if (selected_modules.count(mod_name) > 0)
    return true;
...
bool RTLIL::Selection::selected_member(const RTLIL::IdString &mod_name,
  ...
  for (auto &it : selected_members)
    if (it.second.size() == 0)
      del_list.push_back(it.first);
             ...
             design->modules_[it.first]->wires_.size() +
                 design->modules_[it.first]->memories.size() +
                 design->modules_[it.first]->cells_.size() +
                 design->modules_[it.first]->processes.size())
...
RTLIL::Module *RTLIL::Design::addModule(RTLIL::IdString name) {
  if (modules_.count(name) != 0) {
    // log_error("Attempted to add new module named '%s', but a module by that "
    //           "name already exists\n",
    //           name.c_str());
    exit(1);
  }
...
void RTLIL::Design::scratchpad_set_bool(const std::string &varname,
                                        bool value) {
  scratchpad[varname] = value ? "true" : "false";
}

void RTLIL::Design::scratchpad_set_string(const std::string &varname,
                                          std::string value) {
  scratchpad[varname] = std::move(value);
}

int RTLIL::Design::scratchpad_get_int(const std::string &varname,
...
bool RTLIL::Design::scratchpad_get_bool(const std::string &varname,
...
std::string
RTLIL::Design::scratchpad_get_string(const std::string &varname,
...
bool RTLIL::Design::selected_whole_module(
    const RTLIL::IdString &mod_name) const {
  if (!selected_active_module.empty() && mod_name != selected_active_module)
    return false;
  if (selection_stack.size() == 0)
...
bool RTLIL::Design::selected_member(const RTLIL::IdString &mod_name,
                                    const RTLIL::IdString &memb_name) const {
  if (!selected_active_module.empty() && mod_name != selected_active_module)
    return false;
  if (selection_stack.size() == 0)
...
RTLIL::Design::selected_whole_modules_warn(bool include_wb) const {
  std::vector<RTLIL::Module *> result;
  result.reserve(modules_.size());
  for (auto &it : modules_)
...
void RTLIL::Module::expand_interfaces(
    RTLIL::Design *, const dict<RTLIL::IdString, RTLIL::Module *> &) {
...
RTLIL::Module::derive(RTLIL::Design *,
                      const dict<RTLIL::IdString, RTLIL::Const> &,
                      bool mayfail) {
  if (mayfail)
...
RTLIL::IdString RTLIL::Module::derive(
    RTLIL::Design *, const dict<RTLIL::IdString, RTLIL::Const> &,
    const dict<RTLIL::IdString, RTLIL::Module *> &,
...
size_t RTLIL::Module::count_id(const RTLIL::IdString &id) {
  return wires_.count(id) + memories.count(id) + cells_.count(id) +
         processes.count(id);
}
...
struct InternalCellChecker {
  ...
  InternalCellChecker(RTLIL::Module *module, RTLIL::Cell *cell)
      : module(module), cell(cell) {}
  ...
  void error(int linenr) {
    ...
    //           cell->name.c_str(), cell->type.c_str(), __FILE__, linenr,
    //           buf.str().c_str());
  }
  ...
  void check_expected(bool check_matched_sign = false) {
    ...
    if (check_matched_sign) {
      // log_assert(expected_params.count(ID::A_SIGNED) != 0 &&
      //            expected_params.count(ID::B_SIGNED) != 0);
  ...
  void check() {
    if (!cell->type.begins_with("$") || cell->type.begins_with("$__") ||
        cell->type.begins_with("$paramod") ||
        cell->type.begins_with("$fmcombine") ||
        cell->type.begins_with("$verific$") ||
    ...
    if (cell->type.in(ID($lt), ID($le), ID($eq), ID($ne), ID($eqx), ID($nex),
                      ID($ge), ID($gt))) {
      param_bool(ID::A_SIGNED);
      param_bool(ID::B_SIGNED);
      port(ID::A, param(ID::A_WIDTH));
    ...
    if (cell->type == ID($fsm)) {
      ...
      param_bits(ID::TRANS_TABLE,
                 param(ID::TRANS_NUM) *
                     (2 * param(ID::STATE_NUM_LOG2) + param(ID::CTRL_IN_WIDTH) +
    ...
    if (cell->type == ID($mem)) {
      ...
      param(ID::OFFSET);
      param(ID::INIT);
      param_bits(ID::RD_CLK_ENABLE, std::max(1, param(ID::RD_PORTS)));
      param_bits(ID::RD_CLK_POLARITY, std::max(1, param(ID::RD_PORTS)));
      param_bits(ID::RD_TRANSPARENT, std::max(1, param(ID::RD_PORTS)));
      param_bits(ID::WR_CLK_ENABLE, std::max(1, param(ID::WR_PORTS)));
      param_bits(ID::WR_CLK_POLARITY, std::max(1, param(ID::WR_PORTS)));
    ...
    if (cell->type == ID($mem_v2)) {
      ...
      param_bits(ID::RD_CLK_ENABLE, std::max(1, param(ID::RD_PORTS)));
      param_bits(ID::RD_CLK_POLARITY, std::max(1, param(ID::RD_PORTS)));
      param_bits(ID::RD_TRANSPARENCY_MASK,
                 std::max(1, param(ID::RD_PORTS) * param(ID::WR_PORTS)));
      param_bits(ID::RD_COLLISION_X_MASK,
                 std::max(1, param(ID::RD_PORTS) * param(ID::WR_PORTS)));
      param_bits(ID::RD_WIDE_CONTINUATION, std::max(1, param(ID::RD_PORTS)));
      param_bits(ID::RD_CE_OVER_SRST, std::max(1, param(ID::RD_PORTS)));
      ...
      param_bits(ID::WR_CLK_ENABLE, std::max(1, param(ID::WR_PORTS)));
      param_bits(ID::WR_CLK_POLARITY, std::max(1, param(ID::WR_PORTS)));
      param_bits(ID::WR_WIDE_CONTINUATION, std::max(1, param(ID::WR_PORTS)));
      param_bits(ID::WR_PRIORITY_MASK,
                 std::max(1, param(ID::WR_PORTS) * param(ID::WR_PORTS)));
      port(ID::RD_CLK, param(ID::RD_PORTS));
      port(ID::RD_EN, param(ID::RD_PORTS));
    ...
    if (cell->type.in(ID($assert), ID($assume), ID($live), ID($fair),
                      ID($cover))) {
      port(ID::A, 1);
      port(ID::EN, 1);
      check_expected();
    ...
    if (cell->type.in(ID($_DFFE_NN_), ID($_DFFE_NP_), ID($_DFFE_PN_),
                      ID($_DFFE_PP_))) {
      port(ID::D, 1);
      port(ID::Q, 1);
      port(ID::C, 1);
    ...
    if (cell->type.in(ID($_DFF_NN0_), ID($_DFF_NN1_), ID($_DFF_NP0_),
                      ID($_DFF_NP1_), ID($_DFF_PN0_), ID($_DFF_PN1_),
                      ID($_DFF_PP0_), ID($_DFF_PP1_))) {
      port(ID::D, 1);
      port(ID::Q, 1);
      port(ID::C, 1);
      port(ID::R, 1);
    ...
    if (cell->type.in(ID($_DFFE_NN0N_), ID($_DFFE_NN0P_), ID($_DFFE_NN1N_),
                      ID($_DFFE_NN1P_), ID($_DFFE_NP0N_), ID($_DFFE_NP0P_),
                      ID($_DFFE_NP1N_), ID($_DFFE_NP1P_), ID($_DFFE_PN0N_),
                      ID($_DFFE_PN0P_), ID($_DFFE_PN1N_), ID($_DFFE_PN1P_),
                      ID($_DFFE_PP0N_), ID($_DFFE_PP0P_), ID($_DFFE_PP1N_),
                      ID($_DFFE_PP1P_))) {
      port(ID::D, 1);
      port(ID::Q, 1);
      port(ID::C, 1);
    ...
    if (cell->type.in(ID($_ALDFF_NN_), ID($_ALDFF_NP_), ID($_ALDFF_PN_),
                      ID($_ALDFF_PP_))) {
      port(ID::D, 1);
      port(ID::Q, 1);
      port(ID::C, 1);
    ...
    if (cell->type.in(ID($_ALDFFE_NNN_), ID($_ALDFFE_NNP_), ID($_ALDFFE_NPN_),
                      ID($_ALDFFE_NPP_), ID($_ALDFFE_PNN_), ID($_ALDFFE_PNP_),
    ...
    if (cell->type.in(ID($_DFFSR_NNN_), ID($_DFFSR_NNP_), ID($_DFFSR_NPN_),
                      ID($_DFFSR_NPP_), ID($_DFFSR_PNN_), ID($_DFFSR_PNP_),
    ...
    if (cell->type.in(
            ID($_DFFSRE_NNNN_), ID($_DFFSRE_NNNP_), ID($_DFFSRE_NNPN_),
            ID($_DFFSRE_NNPP_), ID($_DFFSRE_NPNN_), ID($_DFFSRE_NPNP_),
            ID($_DFFSRE_NPPN_), ID($_DFFSRE_NPPP_), ID($_DFFSRE_PNNN_),
            ID($_DFFSRE_PNNP_), ID($_DFFSRE_PNPN_), ID($_DFFSRE_PNPP_),
            ID($_DFFSRE_PPNN_), ID($_DFFSRE_PPNP_), ID($_DFFSRE_PPPN_),
            ID($_DFFSRE_PPPP_))) {
      port(ID::C, 1);
      port(ID::S, 1);
      port(ID::R, 1);
    ...
    if (cell->type.in(ID($_SDFF_NN0_), ID($_SDFF_NN1_), ID($_SDFF_NP0_),
                      ID($_SDFF_NP1_), ID($_SDFF_PN0_), ID($_SDFF_PN1_),
    ...
    if (cell->type.in(
            ID($_SDFFE_NN0N_), ID($_SDFFE_NN0P_), ID($_SDFFE_NN1N_),
            ID($_SDFFE_NN1P_), ID($_SDFFE_NP0N_), ID($_SDFFE_NP0P_),
            ID($_SDFFE_NP1N_), ID($_SDFFE_NP1P_), ID($_SDFFE_PN0N_),
            ID($_SDFFE_PN0P_), ID($_SDFFE_PN1N_), ID($_SDFFE_PN1P_),
            ID($_SDFFE_PP0N_), ID($_SDFFE_PP0P_), ID($_SDFFE_PP1N_),
            ID($_SDFFE_PP1P_), ID($_SDFFCE_NN0N_), ID($_SDFFCE_NN0P_),
            ID($_SDFFCE_NN1N_), ID($_SDFFCE_NN1P_), ID($_SDFFCE_NP0N_),
            ID($_SDFFCE_NP0P_), ID($_SDFFCE_NP1N_), ID($_SDFFCE_NP1P_),
            ID($_SDFFCE_PN0N_), ID($_SDFFCE_PN0P_), ID($_SDFFCE_PN1N_),
            ID($_SDFFCE_PN1P_), ID($_SDFFCE_PP0N_), ID($_SDFFCE_PP0P_),
            ID($_SDFFCE_PP1N_), ID($_SDFFCE_PP1P_))) {
      port(ID::D, 1);
      port(ID::Q, 1);
      port(ID::C, 1);
      port(ID::R, 1);
    ...
    if (cell->type.in(ID($_DLATCH_NN0_), ID($_DLATCH_NN1_), ID($_DLATCH_NP0_),
                      ID($_DLATCH_NP1_), ID($_DLATCH_PN0_), ID($_DLATCH_PN1_),
    ...
    if (cell->type.in(ID($_DLATCHSR_NNN_), ID($_DLATCHSR_NNP_),
                      ID($_DLATCHSR_NPN_), ID($_DLATCHSR_NPP_),
                      ID($_DLATCHSR_PNN_), ID($_DLATCHSR_PNP_),
                      ID($_DLATCHSR_PPN_), ID($_DLATCHSR_PPP_))) {
      port(ID::E, 1);
      port(ID::S, 1);
      port(ID::R, 1);
      port(ID::D, 1);
...
void RTLIL::Module::check() {
  ...
  for (auto &it : cells_) {
    ...
    if (it.second->has_memid()) {
      log_assert(
          memories.count(it.second->parameters.at(ID::MEMID).decode_string()));
...
bool RTLIL::Module::has_memories_warn() const {
  // if (!memories.empty())
  //   log_warning("Ignoring module %s because it contains memories (run
  //   'memory' "
  //               "command first).\n",
  //               log_id(this));
...
bool RTLIL::Module::has_processes_warn() const {
  // if (!processes.empty())
  //   log_warning("Ignoring module %s because it contains processes (run 'proc'
  //   "
  //               "command first).\n",
  //               log_id(this));
    ...
    void operator()(RTLIL::SigSpec &sig) {
      ...
      for (auto &c : sig.chunks_)
        if (c.wire != NULL && wires_p->count(c.wire)) {
          c.wire =
              module->addWire(stringf("$delete_wire$%d", autoidx++), c.width);
          c.offset = 0;
        }
    }
    ...
    void operator()(RTLIL::SigSpec &lhs, RTLIL::SigSpec &rhs) {
      ...
      for (int i = 0; i < GetSize(lhs); i++) {
        ...
        if ((lhs_bit.wire != nullptr && wires_p->count(lhs_bit.wire)) ||
            (rhs_bit.wire != nullptr && wires_p->count(rhs_bit.wire))) {
          lhs_bit = State::Sx;
          rhs_bit = State::Sx;
        }
      }
...
void RTLIL::Module::new_connections(
    const std::vector<RTLIL::SigSig> &new_conn) {
  for (auto mon : monitors)
  ...
  //         log_signal(conn.second), GetSize(conn.first));
  //   log_backtrace("-X- ", yosys_xtrace - 1);
  // }
...
#define DEF_METHOD(_func, _y_size, _type)                                      \
  RTLIL::Cell *RTLIL::Module::add##_func(                                      \
  ...
  RTLIL::SigSpec RTLIL::Module::_func(                                         \
      RTLIL::IdString name, const RTLIL::SigSpec &sig_a, bool is_signed,       \
...
#define DEF_METHOD(_func, _y_size, _type)                                      \
  ...
  RTLIL::SigSpec RTLIL::Module::_func(                                         \
      RTLIL::IdString name, const RTLIL::SigSpec &sig_a,                       \
      const RTLIL::SigSpec &sig_b, bool is_signed, const std::string &src) {   \
    RTLIL::SigSpec sig_y = addWire(NEW_ID, _y_size);                           \
    add##_func(name, sig_a, sig_b, sig_y, is_signed, src);                     \
    return sig_y;                                                              \
  }
DEF_METHOD(And, std::max(sig_a.size(), sig_b.size()), ID($and))
DEF_METHOD(Or, std::max(sig_a.size(), sig_b.size()), ID($or))
DEF_METHOD(Xor, std::max(sig_a.size(), sig_b.size()), ID($xor))
DEF_METHOD(Xnor, std::max(sig_a.size(), sig_b.size()), ID($xnor))
...
DEF_METHOD(Add, std::max(sig_a.size(), sig_b.size()), ID($add))
DEF_METHOD(Sub, std::max(sig_a.size(), sig_b.size()), ID($sub))
DEF_METHOD(Mul, std::max(sig_a.size(), sig_b.size()), ID($mul))
DEF_METHOD(Div, std::max(sig_a.size(), sig_b.size()), ID($div))
DEF_METHOD(Mod, std::max(sig_a.size(), sig_b.size()), ID($mod))
DEF_METHOD(DivFloor, std::max(sig_a.size(), sig_b.size()), ID($divfloor))
DEF_METHOD(ModFloor, std::max(sig_a.size(), sig_b.size()), ID($modfloor))
...
#define DEF_METHOD(_func, _type, _demux)                                       \
  ...
  RTLIL::SigSpec RTLIL::Module::_func(                                         \
    ...
    RTLIL::SigSpec sig_y =                                                     \
        addWire(NEW_ID, _demux ? sig_a.size() << sig_s.size()                  \
                               : sig_a.size() >> sig_s.size());                \
...
#define DEF_METHOD_2(_func, _type, _P1, _P2)                                   \
  RTLIL::Cell *RTLIL::Module::add##_func(                                      \
      RTLIL::IdString name, const RTLIL::SigBit &sig1,                         \
      const RTLIL::SigBit &sig2, const std::string &src) {                     \
    RTLIL::Cell *cell = addCell(name, _type);                                  \
  ...
  RTLIL::SigBit RTLIL::Module::_func(RTLIL::IdString name,                     \
                                     const RTLIL::SigBit &sig1,                \
                                     const std::string &src) {                 \
...
#define DEF_METHOD_3(_func, _type, _P1, _P2, _P3)                              \
  RTLIL::Cell *RTLIL::Module::add##_func(                                      \
      RTLIL::IdString name, const RTLIL::SigBit &sig1,                         \
      const RTLIL::SigBit &sig2, const RTLIL::SigBit &sig3,                    \
      const std::string &src) {                                                \
    RTLIL::Cell *cell = addCell(name, _type);                                  \
    cell->setPort("\\" #_P1, sig1);                                            \
    cell->setPort("\\" #_P2, sig2);                                            \
  ...
  RTLIL::SigBit RTLIL::Module::_func(                                          \
      RTLIL::IdString name, const RTLIL::SigBit &sig1,                         \
      const RTLIL::SigBit &sig2, const std::string &src) {                     \
...
#define DEF_METHOD_5(_func, _type, _P1, _P2, _P3, _P4, _P5)                    \
  RTLIL::Cell *RTLIL::Module::add##_func(                                      \
      RTLIL::IdString name, const RTLIL::SigBit &sig1,                         \
      const RTLIL::SigBit &sig2, const RTLIL::SigBit &sig3,                    \
      const RTLIL::SigBit &sig4, const RTLIL::SigBit &sig5,                    \
      const std::string &src) {                                                \
    RTLIL::Cell *cell = addCell(name, _type);                                  \
    cell->setPort("\\" #_P1, sig1);                                            \
    cell->setPort("\\" #_P2, sig2);                                            \
...
RTLIL::Cell *RTLIL::Module::addPow(RTLIL::IdString name,
                                   ...
                                   const RTLIL::SigSpec &sig_y, bool a_signed,
                                   bool b_signed, const std::string &src) {
...
RTLIL::Cell *
RTLIL::Module::addFa(RTLIL::IdString name, const RTLIL::SigSpec &sig_a,
...
RTLIL::Cell *RTLIL::Module::addLut(RTLIL::IdString name,
                                   const RTLIL::SigSpec &sig_a,
                                   const RTLIL::SigSpec &sig_y,
                                   RTLIL::Const lut, const std::string &src) {
...
RTLIL::Cell *RTLIL::Module::addTribuf(RTLIL::IdString name,
                                      const RTLIL::SigSpec &sig_a,
...
RTLIL::Cell *RTLIL::Module::addAssume(RTLIL::IdString name,
                                      const RTLIL::SigSpec &sig_a,
                                      const RTLIL::SigSpec &sig_en,
                                      const std::string &src) {
...
RTLIL::Cell *RTLIL::Module::addLive(RTLIL::IdString name,
                                    const RTLIL::SigSpec &sig_a,
                                    const RTLIL::SigSpec &sig_en,
                                    const std::string &src) {
...
RTLIL::Cell *RTLIL::Module::addFair(RTLIL::IdString name,
                                    const RTLIL::SigSpec &sig_a,
                                    const RTLIL::SigSpec &sig_en,
                                    const std::string &src) {
...
RTLIL::Cell *RTLIL::Module::addCover(RTLIL::IdString name,
                                     const RTLIL::SigSpec &sig_a,
                                     const RTLIL::SigSpec &sig_en,
                                     const std::string &src) {
...
RTLIL::Cell *RTLIL::Module::addEquiv(RTLIL::IdString name,
                                     const RTLIL::SigSpec &sig_a,
                                     const RTLIL::SigSpec &sig_b,
                                     const RTLIL::SigSpec &sig_y,
                                     const std::string &src) {
...
RTLIL::Cell *RTLIL::Module::addSr(RTLIL::IdString name,
                                  ...
                                  bool set_polarity, bool clr_polarity,
                                  const std::string &src) {
...
RTLIL::Cell *RTLIL::Module::addFf(RTLIL::IdString name,
...
RTLIL::Cell *RTLIL::Module::addDffsr(
    RTLIL::IdString name, const RTLIL::SigSpec &sig_clk,
    const RTLIL::SigSpec &sig_set, const RTLIL::SigSpec &sig_clr,
    RTLIL::SigSpec sig_d, const RTLIL::SigSpec &sig_q, bool clk_polarity,
...
RTLIL::Cell *
RTLIL::Module::addAdff(RTLIL::IdString name, const RTLIL::SigSpec &sig_clk,
                       const RTLIL::SigSpec &sig_arst,
                       const RTLIL::SigSpec &sig_d, const RTLIL::SigSpec &sig_q,
                       RTLIL::Const arst_value, bool clk_polarity,
...
RTLIL::Cell *RTLIL::Module::addAdffe(
    RTLIL::IdString name, const RTLIL::SigSpec &sig_clk,
    const RTLIL::SigSpec &sig_en, const RTLIL::SigSpec &sig_arst,
    const RTLIL::SigSpec &sig_d, const RTLIL::SigSpec &sig_q,
    RTLIL::Const arst_value, bool clk_polarity, bool en_polarity,
    bool arst_polarity, const std::string &src) {
...
RTLIL::Cell *RTLIL::Module::addAldffe(
    RTLIL::IdString name, const RTLIL::SigSpec &sig_clk,
    const RTLIL::SigSpec &sig_en, const RTLIL::SigSpec &sig_aload,
    const RTLIL::SigSpec &sig_d, const RTLIL::SigSpec &sig_q,
    const RTLIL::SigSpec &sig_ad, bool clk_polarity, bool en_polarity,
...
RTLIL::Cell *
RTLIL::Module::addSdff(RTLIL::IdString name, const RTLIL::SigSpec &sig_clk,
                       const RTLIL::SigSpec &sig_srst,
                       const RTLIL::SigSpec &sig_d, const RTLIL::SigSpec &sig_q,
                       RTLIL::Const srst_value, bool clk_polarity,
...
RTLIL::Cell *RTLIL::Module::addSdffe(
    RTLIL::IdString name, const RTLIL::SigSpec &sig_clk,
    const RTLIL::SigSpec &sig_en, const RTLIL::SigSpec &sig_srst,
    const RTLIL::SigSpec &sig_d, const RTLIL::SigSpec &sig_q,
    RTLIL::Const srst_value, bool clk_polarity, bool en_polarity,
    bool srst_polarity, const std::string &src) {
...
RTLIL::Cell *RTLIL::Module::addSdffce(
    RTLIL::IdString name, const RTLIL::SigSpec &sig_clk,
    const RTLIL::SigSpec &sig_en, const RTLIL::SigSpec &sig_srst,
    const RTLIL::SigSpec &sig_d, const RTLIL::SigSpec &sig_q,
    RTLIL::Const srst_value, bool clk_polarity, bool en_polarity,
...
RTLIL::Cell *RTLIL::Module::addAdlatch(
    RTLIL::IdString name, const RTLIL::SigSpec &sig_en,
    const RTLIL::SigSpec &sig_arst, const RTLIL::SigSpec &sig_d,
    const RTLIL::SigSpec &sig_q, RTLIL::Const arst_value, bool en_polarity,
    bool arst_polarity, const std::string &src) {
...
RTLIL::Cell *RTLIL::Module::addDlatchsr(
    RTLIL::IdString name, const RTLIL::SigSpec &sig_en,
    const RTLIL::SigSpec &sig_set, const RTLIL::SigSpec &sig_clr,
    RTLIL::SigSpec sig_d, const RTLIL::SigSpec &sig_q, bool en_polarity,
    bool set_polarity, bool clr_polarity, const std::string &src) {
...
RTLIL::Cell *RTLIL::Module::addSrGate(RTLIL::IdString name,
                                      const RTLIL::SigSpec &sig_set,
                                      const RTLIL::SigSpec &sig_clr,
                                      const RTLIL::SigSpec &sig_q,
                                      bool set_polarity, bool clr_polarity,
                                      const std::string &src) {
  RTLIL::Cell *cell =
      addCell(name, stringf("$_SR_%c%c_", set_polarity ? 'P' : 'N',
                            clr_polarity ? 'P' : 'N'));
...
RTLIL::Cell *RTLIL::Module::addFfGate(RTLIL::IdString name,
                                      const RTLIL::SigSpec &sig_d,
                                      const RTLIL::SigSpec &sig_q,
                                      const std::string &src) {
...
RTLIL::Cell *RTLIL::Module::addDffGate(RTLIL::IdString name,
                                       ...
                                       const std::string &src) {
  RTLIL::Cell *cell =
      addCell(name, stringf("$_DFF_%c_", clk_polarity ? 'P' : 'N'));
...
RTLIL::Cell *RTLIL::Module::addDffeGate(RTLIL::IdString name,
                                        ...
                                        const std::string &src) {
  RTLIL::Cell *cell =
      addCell(name, stringf("$_DFFE_%c%c_", clk_polarity ? 'P' : 'N',
                            en_polarity ? 'P' : 'N'));
  cell->setPort(ID::C, sig_clk);
...
RTLIL::Cell *RTLIL::Module::addDffsrGate(
    RTLIL::IdString name, const RTLIL::SigSpec &sig_clk,
    const RTLIL::SigSpec &sig_set, const RTLIL::SigSpec &sig_clr,
    RTLIL::SigSpec sig_d, const RTLIL::SigSpec &sig_q, bool clk_polarity,
    bool set_polarity, bool clr_polarity, const std::string &src) {
  RTLIL::Cell *cell = addCell(
      name, stringf("$_DFFSR_%c%c%c_", clk_polarity ? 'P' : 'N',
                    set_polarity ? 'P' : 'N', clr_polarity ? 'P' : 'N'));
  cell->setPort(ID::C, sig_clk);
  cell->setPort(ID::S, sig_set);
  cell->setPort(ID::R, sig_clr);
  cell->setPort(ID::D, sig_d);
...
RTLIL::Cell *RTLIL::Module::addDffsreGate(
    RTLIL::IdString name, const RTLIL::SigSpec &sig_clk,
    const RTLIL::SigSpec &sig_en, const RTLIL::SigSpec &sig_set,
    const RTLIL::SigSpec &sig_clr, RTLIL::SigSpec sig_d,
    const RTLIL::SigSpec &sig_q, bool clk_polarity, bool en_polarity,
    bool set_polarity, bool clr_polarity, const std::string &src) {
  RTLIL::Cell *cell =
      addCell(name, stringf("$_DFFSRE_%c%c%c%c_", clk_polarity ? 'P' : 'N',
                            set_polarity ? 'P' : 'N', clr_polarity ? 'P' : 'N',
                            en_polarity ? 'P' : 'N'));
  cell->setPort(ID::C, sig_clk);
  cell->setPort(ID::S, sig_set);
...
RTLIL::Cell *RTLIL::Module::addAdffGate(
    RTLIL::IdString name, const RTLIL::SigSpec &sig_clk,
    const RTLIL::SigSpec &sig_arst, const RTLIL::SigSpec &sig_d,
    const RTLIL::SigSpec &sig_q, bool arst_value, bool clk_polarity,
    bool arst_polarity, const std::string &src) {
  RTLIL::Cell *cell =
      addCell(name, stringf("$_DFF_%c%c%c_", clk_polarity ? 'P' : 'N',
                            arst_polarity ? 'P' : 'N', arst_value ? '1' : '0'));
  cell->setPort(ID::C, sig_clk);
...
RTLIL::Cell *RTLIL::Module::addAdffeGate(
    RTLIL::IdString name, const RTLIL::SigSpec &sig_clk,
    const RTLIL::SigSpec &sig_en, const RTLIL::SigSpec &sig_arst,
    const RTLIL::SigSpec &sig_d, const RTLIL::SigSpec &sig_q, bool arst_value,
    bool clk_polarity, bool en_polarity, bool arst_polarity,
    const std::string &src) {
  RTLIL::Cell *cell =
      addCell(name, stringf("$_DFFE_%c%c%c%c_", clk_polarity ? 'P' : 'N',
                            arst_polarity ? 'P' : 'N', arst_value ? '1' : '0',
                            en_polarity ? 'P' : 'N'));
  cell->setPort(ID::C, sig_clk);
  cell->setPort(ID::R, sig_arst);
...
RTLIL::Cell *RTLIL::Module::addAldffGate(
    RTLIL::IdString name, const RTLIL::SigSpec &sig_clk,
    const RTLIL::SigSpec &sig_aload, const RTLIL::SigSpec &sig_d,
    const RTLIL::SigSpec &sig_q, const RTLIL::SigSpec &sig_ad,
    bool clk_polarity, bool aload_polarity, const std::string &src) {
  RTLIL::Cell *cell =
      addCell(name, stringf("$_ALDFF_%c%c_", clk_polarity ? 'P' : 'N',
                            aload_polarity ? 'P' : 'N'));
  cell->setPort(ID::C, sig_clk);
...
RTLIL::Cell *RTLIL::Module::addAldffeGate(
    RTLIL::IdString name, const RTLIL::SigSpec &sig_clk,
    const RTLIL::SigSpec &sig_en, const RTLIL::SigSpec &sig_aload,
    const RTLIL::SigSpec &sig_d, const RTLIL::SigSpec &sig_q,
    const RTLIL::SigSpec &sig_ad, bool clk_polarity, bool en_polarity,
    bool aload_polarity, const std::string &src) {
  RTLIL::Cell *cell = addCell(
      name, stringf("$_ALDFFE_%c%c%c_", clk_polarity ? 'P' : 'N',
                    aload_polarity ? 'P' : 'N', en_polarity ? 'P' : 'N'));
  cell->setPort(ID::C, sig_clk);
  cell->setPort(ID::L, sig_aload);
  cell->setPort(ID::E, sig_en);
  cell->setPort(ID::D, sig_d);
...
RTLIL::Cell *RTLIL::Module::addSdffGate(
    RTLIL::IdString name, const RTLIL::SigSpec &sig_clk,
    const RTLIL::SigSpec &sig_srst, const RTLIL::SigSpec &sig_d,
    const RTLIL::SigSpec &sig_q, bool srst_value, bool clk_polarity,
    bool srst_polarity, const std::string &src) {
  RTLIL::Cell *cell =
      addCell(name, stringf("$_SDFF_%c%c%c_", clk_polarity ? 'P' : 'N',
                            srst_polarity ? 'P' : 'N', srst_value ? '1' : '0'));
  cell->setPort(ID::C, sig_clk);
...
RTLIL::Cell *RTLIL::Module::addSdffeGate(
    RTLIL::IdString name, const RTLIL::SigSpec &sig_clk,
    const RTLIL::SigSpec &sig_en, const RTLIL::SigSpec &sig_srst,
    const RTLIL::SigSpec &sig_d, const RTLIL::SigSpec &sig_q, bool srst_value,
    bool clk_polarity, bool en_polarity, bool srst_polarity,
    const std::string &src) {
  RTLIL::Cell *cell =
      addCell(name, stringf("$_SDFFE_%c%c%c%c_", clk_polarity ? 'P' : 'N',
                            srst_polarity ? 'P' : 'N', srst_value ? '1' : '0',
                            en_polarity ? 'P' : 'N'));
  cell->setPort(ID::C, sig_clk);
  cell->setPort(ID::R, sig_srst);
...
RTLIL::Cell *RTLIL::Module::addSdffceGate(
    RTLIL::IdString name, const RTLIL::SigSpec &sig_clk,
    const RTLIL::SigSpec &sig_en, const RTLIL::SigSpec &sig_srst,
    const RTLIL::SigSpec &sig_d, const RTLIL::SigSpec &sig_q, bool srst_value,
    bool clk_polarity, bool en_polarity, bool srst_polarity,
    const std::string &src) {
  RTLIL::Cell *cell =
      addCell(name, stringf("$_SDFFCE_%c%c%c%c_", clk_polarity ? 'P' : 'N',
                            srst_polarity ? 'P' : 'N', srst_value ? '1' : '0',
                            en_polarity ? 'P' : 'N'));
  cell->setPort(ID::C, sig_clk);
  cell->setPort(ID::R, sig_srst);
...
RTLIL::Cell *RTLIL::Module::addDlatchGate(RTLIL::IdString name,
                                          ...
                                          const std::string &src) {
  RTLIL::Cell *cell =
      addCell(name, stringf("$_DLATCH_%c_", en_polarity ? 'P' : 'N'));
  cell->setPort(ID::E, sig_en);
  cell->setPort(ID::D, sig_d);
  cell->setPort(ID::Q, sig_q);
  cell->set_src_attribute(src);
...
RTLIL::Cell *RTLIL::Module::addAdlatchGate(
    RTLIL::IdString name, const RTLIL::SigSpec &sig_en,
    const RTLIL::SigSpec &sig_arst, const RTLIL::SigSpec &sig_d,
    const RTLIL::SigSpec &sig_q, bool arst_value, bool en_polarity,
    bool arst_polarity, const std::string &src) {
  RTLIL::Cell *cell =
      addCell(name, stringf("$_DLATCH_%c%c%c_", en_polarity ? 'P' : 'N',
                            arst_polarity ? 'P' : 'N', arst_value ? '1' : '0'));
  cell->setPort(ID::E, sig_en);
  cell->setPort(ID::R, sig_arst);
  cell->setPort(ID::D, sig_d);
...
RTLIL::Cell *RTLIL::Module::addDlatchsrGate(
    RTLIL::IdString name, const RTLIL::SigSpec &sig_en,
    const RTLIL::SigSpec &sig_set, const RTLIL::SigSpec &sig_clr,
    RTLIL::SigSpec sig_d, const RTLIL::SigSpec &sig_q, bool en_polarity,
    bool set_polarity, bool clr_polarity, const std::string &src) {
  RTLIL::Cell *cell = addCell(
      name, stringf("$_DLATCHSR_%c%c%c_", en_polarity ? 'P' : 'N',
                    set_polarity ? 'P' : 'N', clr_polarity ? 'P' : 'N'));
  cell->setPort(ID::E, sig_en);
  cell->setPort(ID::S, sig_set);
  cell->setPort(ID::R, sig_clr);
...
RTLIL::Cell *RTLIL::Module::addAnyinit(RTLIL::IdString name,
                                       const RTLIL::SigSpec &sig_d,
                                       const RTLIL::SigSpec &sig_q,
                                       const std::string &src) {
...
RTLIL::SigSpec RTLIL::Module::Allconst(RTLIL::IdString name, int width,
                                       const std::string &src) {
  RTLIL::SigSpec sig = addWire(NEW_ID, width);
...
RTLIL::SigSpec RTLIL::Module::Allseq(RTLIL::IdString name, int width,
                                     const std::string &src) {
  RTLIL::SigSpec sig = addWire(NEW_ID, width);
...
RTLIL::SigSpec
RTLIL::Module::SetTag(RTLIL::IdString name, const std::string &tag,
                      const RTLIL::SigSpec &sig_a, const RTLIL::SigSpec &sig_s,
...
RTLIL::Cell *RTLIL::Module::addSetTag(
    RTLIL::IdString name, const std::string &tag, const RTLIL::SigSpec &sig_a,
    const RTLIL::SigSpec &sig_s, const RTLIL::SigSpec &sig_c,
    const RTLIL::SigSpec &sig_y, const std::string &src) {
  Cell *cell = addCell(name, ID($set_tag));
  cell->parameters[ID::WIDTH] = sig_a.size();
  cell->parameters[ID::TAG] = tag;
...
RTLIL::SigSpec RTLIL::Module::GetTag(RTLIL::IdString name,
...
RTLIL::Cell *RTLIL::Module::addOverwriteTag(RTLIL::IdString name,
                                            const std::string &tag,
                                            const RTLIL::SigSpec &sig_a,
...
RTLIL::SigSpec RTLIL::Module::OriginalTag(RTLIL::IdString name,
                                          const std::string &tag,
                                          const RTLIL::SigSpec &sig_a,
                                          const std::string &src) {
  RTLIL::SigSpec sig = addWire(NEW_ID, sig_a.size());
...
RTLIL::SigSpec RTLIL::Module::FutureFF(RTLIL::IdString name,
                                       const RTLIL::SigSpec &sig_e,
                                       const std::string &src) {
...
void RTLIL::Cell::unsetPort(const RTLIL::IdString &portname) {
  ...
  if (conn_it != connections_.end()) {
    ...
    // if (yosys_xtrace) {
    //   log("#X# Unconnect %s.%s.%s\n", log_id(this->module), log_id(this),
    //       log_id(portname));
    //   log_backtrace("-X- ", yosys_xtrace - 1);
    // }
...
void RTLIL::Cell::setPort(const RTLIL::IdString &portname,
                          RTLIL::SigSpec signal) {
  ...
  //   log_id(this),
  //       log_id(portname), log_signal(signal), GetSize(signal));
  //   log_backtrace("-X- ", yosys_xtrace - 1);
  // }
...
const RTLIL::SigSpec &
RTLIL::Cell::getPort(const RTLIL::IdString &portname) const {
...
void RTLIL::Cell::setParam(const RTLIL::IdString &paramname,
                           RTLIL::Const value) {
  parameters[paramname] = std::move(value);
}

const RTLIL::Const &
RTLIL::Cell::getParam(const RTLIL::IdString &paramname) const {
...
void RTLIL::Cell::fixup_parameters(bool set_a_signed, bool set_b_signed) {
  if (!type.begins_with("$") || type.begins_with("$_") ||
      type.begins_with("$paramod") || type.begins_with("$fmcombine") ||
      type.begins_with("$verific$") || type.begins_with("$array:") ||
      type.begins_with("$extern:"))
    return;
...
bool RTLIL::Cell::has_memid() const {
  return type.in(ID($memwr), ID($memwr_v2), ID($memrd), ID($memrd_v2),
                 ID($meminit), ID($meminit_v2));
}
...
bool RTLIL::SigChunk::operator==(const RTLIL::SigChunk &other) const {
  return wire == other.wire && width == other.width && offset == other.offset &&
         data == other.data;
}
...
void RTLIL::SigSpec::remove2(const pool<RTLIL::SigBit> &pattern,
                             RTLIL::SigSpec *other) {
  if (other)
    cover("kernel.rtlil.sigspec.remove_other");
  else
...
RTLIL::SigSpec RTLIL::SigSpec::extract(const RTLIL::SigSpec &pattern,
                                       const RTLIL::SigSpec *other) const {
  if (other)
  ...
  for (auto &pattern_chunk : pattern.chunks()) {
    if (other) {
      ...
      for (int i = 0; i < width_; i++)
        if (bits_match[i].wire && bits_match[i].wire == pattern_chunk.wire &&
...
RTLIL::SigSpec RTLIL::SigSpec::extract(const pool<RTLIL::SigBit> &pattern,
                                       const RTLIL::SigSpec *other) const {
  if (other)
    cover("kernel.rtlil.sigspec.extract_other");
  else
    ...
    for (auto &chunk : chunks_)
      if (chunk.wire != NULL) {
        if (!new_chunks.empty() && new_chunks.back().wire == chunk.wire &&
            new_chunks.back().offset + new_chunks.back().width ==
                chunk.offset) {
...
void RTLIL::SigSpec::append(const RTLIL::SigSpec &signal) {
  ...
  if (packed())
    for (auto &other_c : signal.chunks_) {
      ...
      } else if (my_last_c.wire == other_c.wire &&
  ...
  if (packed()) {
    ...
    if (chunks_.size() == 0)
      ...
      if (chunks_.back().wire == NULL) {
        ...
        chunks_.push_back(bit);
    else if (chunks_.back().wire == bit.wire &&
    ...
    for (size_t i = 0; i < chunks_.size(); i++) {
      ...
      if (chunk.wire == NULL) {
        ...
        if (i > 0 && chunks_[i - 1].wire == chunk.wire)
          log_assert(chunk.offset !=
                     chunks_[i - 1].offset + chunks_[i - 1].width);
        log_assert(chunk.offset >= 0);
        log_assert(chunk.width >= 0);
...
bool RTLIL::SigSpec::is_wire() const {
  ...
  pack();
  return GetSize(chunks_) == 1 && chunks_[0].wire &&
         chunks_[0].wire->width == width_;
}
...
RTLIL::SigSpec::to_sigbit_map(const RTLIL::SigSpec &other) const {
...
RTLIL::SigSpec::to_sigbit_dict(const RTLIL::SigSpec &other) const {
...
bool RTLIL::SigSpec::parse(RTLIL::SigSpec &sig, RTLIL::Module *module,
                           std::string str) {
  ...
  for (int tokidx = int(tokens.size()) - 1; tokidx >= 0; tokidx--) {
    ...
    if (module->wires_.count(netname) == 0) {
      ...
      if (indices_pos > 2 && netname[indices_pos] == ']') {
        indices_pos--;
        while (indices_pos > 0 &&
               ('0' <= netname[indices_pos] && netname[indices_pos] <= '9'))
          indices_pos--;
...
bool RTLIL::SigSpec::parse_sel(RTLIL::SigSpec &sig, RTLIL::Design *design,
...
bool RTLIL::SigSpec::parse_rhs(const RTLIL::SigSpec &lhs, RTLIL::SigSpec &sig,
                               RTLIL::Module *module, std::string str) {
  if (str == "0") {
...
static BigInteger const2big(const RTLIL::Const &val, bool as_signed,
                            int &undef_bit_pos) {
  BigUnsigned mag;
...
static RTLIL::Const big2const(const BigInteger &val, int result_len,
                              int undef_bit_pos) {
  if (undef_bit_pos >= 0)
...
RTLIL::Const RTLIL::const_not(const RTLIL::Const &arg1, const RTLIL::Const &,
...
static RTLIL::Const
logic_wrapper(RTLIL::State (*logic_func)(RTLIL::State, RTLIL::State),
              RTLIL::Const arg1, RTLIL::Const arg2, bool signed1, bool signed2,
              int result_len = -1) {
  if (result_len < 0)
...
RTLIL::Const RTLIL::const_or(const RTLIL::Const &arg1, const RTLIL::Const &arg2,
...
logic_reduce_wrapper(RTLIL::State initial,
                     RTLIL::State (*logic_func)(RTLIL::State, RTLIL::State),
                     const RTLIL::Const &arg1, int result_len) {
...
RTLIL::Const RTLIL::const_logic_not(const RTLIL::Const &arg1,
                                    const RTLIL::Const &, bool signed1, bool,
                                    int result_len) {
  int undef_bit_pos_a = -1;
  BigInteger a = const2big(arg1, signed1, undef_bit_pos_a);
  RTLIL::Const result(a.isZero() ? undef_bit_pos_a >= 0 ? RTLIL::State::Sx
                                                        : RTLIL::State::S1
                                 : RTLIL::State::S0);
...
// If `direction` is +1, `arg1` is shifted right by `arg2` bits; if `direction`
// is -1, `arg1` is shifted left by `arg2` bits. If `signed2` is true, `arg2` is
// interpreted as a signed integer; a negative `arg2` will cause a shift in the
// opposite direction. Any required bits outside the bounds of `arg1` are padded
// with `vacant_bits` unless `sign_ext` is true, in which case any bits outside
// the left bounds are filled with the leftmost bit of `arg1` (arithmetic
// shift).
static RTLIL::Const
const_shift_worker(const RTLIL::Const &arg1, const RTLIL::Const &arg2,
                   bool sign_ext, bool signed2, int direction, int result_len,
...
RTLIL::Const RTLIL::const_shl(const RTLIL::Const &arg1,
                              const RTLIL::Const &arg2, bool signed1, bool,
                              int result_len) {
  RTLIL::Const arg1_ext = arg1;
  extend_u0(arg1_ext, result_len, signed1);
...
RTLIL::Const RTLIL::const_shiftx(const RTLIL::Const &arg1,
                                 const RTLIL::Const &arg2, bool, bool signed2,
                                 int result_len) {
  return const_shift_worker(arg1, arg2, false, signed2, +1, result_len,
                            RTLIL::State::Sx);
}

RTLIL::Const RTLIL::const_lt(const RTLIL::Const &arg1, const RTLIL::Const &arg2,
                             bool signed1, bool signed2, int result_len) {
  int undef_bit_pos = -1;
  bool y = const2big(arg1, signed1, undef_bit_pos) <
           const2big(arg2, signed2, undef_bit_pos);
  RTLIL::Const result(undef_bit_pos >= 0 ? RTLIL::State::Sx
                      : y                ? RTLIL::State::S1
                                         : RTLIL::State::S0);
...
RTLIL::Const RTLIL::const_le(const RTLIL::Const &arg1, const RTLIL::Const &arg2,
                             bool signed1, bool signed2, int result_len) {
  int undef_bit_pos = -1;
  bool y = const2big(arg1, signed1, undef_bit_pos) <=
           const2big(arg2, signed2, undef_bit_pos);
...
RTLIL::Const RTLIL::const_eq(const RTLIL::Const &arg1, const RTLIL::Const &arg2,
                             bool signed1, bool signed2, int result_len) {
  RTLIL::Const arg1_ext = arg1;
...
RTLIL::Const RTLIL::const_ne(const RTLIL::Const &arg1, const RTLIL::Const &arg2,
                             bool signed1, bool signed2, int result_len) {
  RTLIL::Const result =
      RTLIL::const_eq(arg1, arg2, signed1, signed2, result_len);
...
RTLIL::Const RTLIL::const_nex(const RTLIL::Const &arg1,
                              ...
                              bool signed2, int result_len) {
  RTLIL::Const result =
      RTLIL::const_eqx(arg1, arg2, signed1, signed2, result_len);
  if (result.bits.front() == RTLIL::State::S0)
    result.bits.front() = RTLIL::State::S1;
  else if (result.bits.front() == RTLIL::State::S1)
    result.bits.front() = RTLIL::State::S0;
...
RTLIL::Const RTLIL::const_ge(const RTLIL::Const &arg1, const RTLIL::Const &arg2,
                             bool signed1, bool signed2, int result_len) {
  int undef_bit_pos = -1;
  bool y = const2big(arg1, signed1, undef_bit_pos) >=
           const2big(arg2, signed2, undef_bit_pos);
...
RTLIL::Const RTLIL::const_gt(const RTLIL::Const &arg1, const RTLIL::Const &arg2,
                             bool signed1, bool signed2, int result_len) {
  int undef_bit_pos = -1;
  bool y = const2big(arg1, signed1, undef_bit_pos) >
           const2big(arg2, signed2, undef_bit_pos);
...
RTLIL::Const RTLIL::const_add(const RTLIL::Const &arg1,
  ...
  return big2const(y,
                   result_len >= 0
                       ? result_len
                       : std::max(arg1.bits.size(), arg2.bits.size()),
                   undef_bit_pos);
}
...
RTLIL::Const RTLIL::const_divfloor(const RTLIL::Const &arg1,
                                   const RTLIL::Const &arg2, bool signed1,
                                   bool signed2, int result_len) {
  int undef_bit_pos = -1;
  BigInteger a = const2big(arg1, signed1, undef_bit_pos);
  ...
  bool result_pos = (a.getSign() == BigInteger::negative) ==
                    (b.getSign() == BigInteger::negative);
  a = a.getSign() == BigInteger::negative ? -a : a;
  b = b.getSign() == BigInteger::negative ? -b : b;
  BigInteger result;
  ...
  if (result_pos || a == 0) {
    ...
    // bigint division with negative numbers is wonky, make sure we only negate
    // at the very end
    result = -((a + b - 1) / b);
  }
  return big2const(result,
                   result_len >= 0
                       ? result_len
                       : std::max(arg1.bits.size(), arg2.bits.size()),
                   std::min(undef_bit_pos, 0));
...
RTLIL::Const RTLIL::const_modfloor(const RTLIL::Const &arg1,
                                   const RTLIL::Const &arg2, bool signed1,
                                   bool signed2, int result_len) {
  int undef_bit_pos = -1;
  BigInteger a = const2big(arg1, signed1, undef_bit_pos);
...
RTLIL::Const RTLIL::const_pos(const RTLIL::Const &arg1, const RTLIL::Const &,
                              bool signed1, bool, int result_len) {
  RTLIL::Const arg1_ext = arg1;
  extend_u0(arg1_ext, result_len, signed1);
...
RTLIL::Const RTLIL::const_neg(const RTLIL::Const &arg1, const RTLIL::Const &,
                              bool signed1, bool, int result_len) {
  RTLIL::Const arg1_ext = arg1;
...
RTLIL::Const RTLIL::const_mux(const RTLIL::Const &arg1,
...
RTLIL::Const RTLIL::const_pmux(const RTLIL::Const &arg1,
                               const RTLIL::Const &arg2,
                               const RTLIL::Const &arg3) {
  ...
  for (int i = 0; i < arg3.size(); i++)
    if (arg3[i] == State::S1)
      return RTLIL::Const(std::vector<RTLIL::State>(
          arg2.bits.begin() + i * arg1.bits.size(),
          arg2.bits.begin() + (i + 1) * arg1.bits.size()));

  // log_abort(); // unreachable
  exit(1);
}

RTLIL::Const RTLIL::const_bmux(const RTLIL::Const &arg1,
                               const RTLIL::Const &arg2) {
...
RTLIL::Const RTLIL::const_demux(const RTLIL::Const &arg1,
                                const RTLIL::Const &arg2) {
...
RTLIL::Const RTLIL::const_bweqx(const RTLIL::Const &arg1,
                                const RTLIL::Const &arg2) {
  // log_assert(arg2.size() == arg1.size());
...
RTLIL::Const RTLIL::const_bwmux(const RTLIL::Const &arg1,
                                const RTLIL::Const &arg2,
                                const RTLIL::Const &arg3) {
  // log_assert(arg2.size() == arg1.size());
